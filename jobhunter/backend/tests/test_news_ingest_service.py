...
async def test_ingest_creates_funding_signals(db_session):
    news = AsyncMock()
    news.search_articles = AsyncMock(
        return_value=[
            {
    ...
    openai = AsyncMock()
    openai.parse_structured = AsyncMock(
        return_value={
            "companies": [
                {
                    "company_name": "Acme",
    ...
    assert count == 1
    sig = (await db_session.execute(select(FundingSignal).where(FundingSignal.source_url == article_url))).scalar_one()
...
async def test_ingest_deduplicates_on_reruns(db_session):
    ...
    total = (await db_session.execute(select(FundingSignal).where(FundingSignal.source_url == dup_url))).scalars().all()
    assert len(total) == 1
