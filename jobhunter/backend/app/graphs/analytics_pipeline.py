...
INSIGHTS_PROMPT = (
    "You are a job search analytics advisor. Analyze this candidate's job search data and generate actionable insights.\n\n"
    "Pipeline: {pipeline}\n"
    ...
    "- insight_type: one of 'pipeline_health', 'outreach_effectiveness', 'skill_gap', 'market_positioning', 'recommendation'\n"
    "- title: short descriptive title (under 100 chars)\n"
    "- body: detailed explanation with specific numbers and recommendations (2-3 sentences)\n"
...
async def gather_data_node(state: AnalyticsState) -> dict:
    ...
    try:
        async with _db_mod.async_session_factory() as db:
            ...
            from app.models.candidate import Skill, CandidateDNA
...
async def notify_node(state: AnalyticsState) -> dict:
    ...
    await ws_manager.broadcast(
        state["candidate_id"], "analytics_completed",
    ...
    if state.get("include_email"):
        try:
            ...
            for i, insight in enumerate(insights, 1):
                lines.append(
                    f"{i}. [{insight.get('severity', 'info').upper()}] "
                    f"{insight.get('title', '')}"
                )
            ...
            async with _db_mod.async_session_factory() as db:
                ...
                result = await db.execute(
                    select(Candidate).where(
                        Candidate.id == uuid.UUID(state["candidate_id"])
                    )
                )
...
async def mark_failed_node(state: AnalyticsState) -> dict:
    ...
    await ws_manager.broadcast(
        state["candidate_id"], "analytics_failed",
